...
#define DMA_UR1R_CLRFIFO()			DMA_UR1R_CR |= 0x01
...
#define SET_M2M_TX_FIFO(n)			DMA_M2M_TXA = (n)
#define SET_M2M_RX_FIFO(n)			DMA_M2M_RXA = (n)
...
typedef struct
{
	...
	u8	DMA_Length;					//DMA传输总字节数  	(0~255) + 1
...
typedef struct
{
	...
	u8	DMA_TX_Length;			//DMA传输总字节数  	(0~255) + 1
	u16	DMA_TX_Buffer;			//发送数据存储地址

	u8	DMA_RX_Enable;			//DMA使能  	ENABLE,DISABLE
	u8	DMA_RX_Length;			//DMA传输总字节数  	(0~255) + 1
	u16	DMA_RX_Buffer;			//接收数据存储地址
